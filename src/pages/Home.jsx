    ...
    <div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center gap-2 mb-4">
          <BadgeAlert size={20} className="text-[#003D73]" />
          <h2 className="text-lg font-bold text-gray-800">Aktuelle Hinweise</h2>
        </div>
        <div className="flex flex-col gap-2">
          {alerts.map((a, i) => (
            <div key={i} className={`flex items-start gap-3 px-4 py-3 rounded-xl text-sm font-medium ${
              a.type === 'warn' ? 'bg-amber-50 text-amber-800 border border-amber-200'
              : a.type === 'ok' ? 'bg-green-50 text-green-800 border border-green-200'
      ...
      <section className="max-w-7xl mx-auto px-4 pb-8">
        <h2 className="text-xl font-bold text-gray-900 mb-2 text-center">Live Updates</h2>
        <p className="text-gray-500 text-sm text-center mb-6">Aktuelle Informationen auf einen Blick</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: <Bell size={22} />, color: 'bg-red-100 text-red-600', count: '3', label: 'AKTIVE MELDUNGEN' },
            { icon: <Calendar size={22} />, color: 'bg-blue-100 text-blue-600', count: '12', label: 'VERANSTALTUNGEN' },
            { icon: <Newspaper size={22} />, color: 'bg-purple-100 text-purple-600', count: '8', label: 'NACHRICHTEN' },
            { icon: <Wrench size={22} />, color: 'bg-green-100 text-green-600', count: '24', label: 'SERVICEANFRAGEN' },
          ].map(c => (
            <div key={c.label} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
              ...
              <div className="text-3xl font-bold text-gray-900 mb-1">{c.count}</div>
      ...
      {/* Quick Services */}
      <section className="bg-[#E8F0F9] py-10">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-gray-900">Stadtservices</h2>
              <p className="text-gray-500 text-sm">Direktzugang zu Behörden & Einrichtungen</p>
            </div>
            <Link to="/services" className="flex items-center gap-1 text-[#003D73] font-semibold text-sm hover:underline">
              Alle anzeigen <ChevronRight size={16} />
            </Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {quickServices.map(s => (
              <div key={s.label} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md hover:border-[#003D73]/30 transition-all cursor-pointer group">
                <div className="w-11 h-11 rounded-xl bg-[#003D73]/10 text-[#003D73] flex items-center justify-center mb-3 group-hover:bg-[#003D73] group-hover:text-white transition-colors">{s.icon}</div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Veranstaltungen</h2>
            <p className="text-gray-500 text-sm">Was ist los in Aachen?</p>
          </div>
          <Link to="/events" className="flex items-center gap-1 text-[#003D73] font-semibold text-sm hover:underline">
            Alle Termine <ChevronRight size={16} />
          </Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {upcomingEvents.map(e => (
            <div key={e.title} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow cursor-pointer">
              ...
              <div className="p-4">
                <span className="text-xs font-semibold text-[#003D73] bg-blue-50 px-2 py-0.5 rounded-full">{e.cat}</span>
                <div className="font-semibold text-gray-900 text-sm mt-2 mb-1">{e.title}</div>
